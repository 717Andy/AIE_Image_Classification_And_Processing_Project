...
import numpy as np
import cv2  # Used to overlay the heatmap onto the original image

# 1. Load the model
model = MobileNetV2(weights="imagenet")

def make_gradcam_heatmap(img_array, model, last_conv_layer_name, pred_index=None):
    """Calculates the Grad-CAM heatmap for a given image array."""
...
def save_and_display_gradcam(img_path, heatmap, cam_path="cam.jpg", alpha=0.4):
    ...
    # Convert heatmap to a RGB color map
    ...
    print(f"  [Grad-CAM Visual Saved as: {cam_path}]")
...
def classify_image(image_path):
    try:
        ...
        # --- GRAD-CAM GENERATION ---
        # 'Out_Relu' is the final convolutional/activation layer in MobileNetV2
        last_conv_layer_name = "out_relu" 
        heatmap = make_gradcam_heatmap(img_array, model, last_conv_layer_name)
        
        # Save output image alongside original name (e.g., 'dog.jpg' -> 'gradcam_dog.jpg')
        output_filename = f"gradcam_{image_path}"
        save_and_display_gradcam(image_path, heatmap, cam_path=output_filename)
